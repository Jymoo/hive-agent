...
func (r *Runner) once(ctx context.Context) {
	...
	if err := r.api.Heartbeat(ctx, payload); err != nil {
		if errors.Is(err, registration.ErrUnauthorized) {
			// JWT was rejected by the backend — re-register to get a fresh token.
			r.log.Warn("heartbeat JWT rejected (401/403); attempting re-registration", zap.Error(err))
			if reErr := r.api.Reregister(ctx); reErr != nil {
				r.log.Error("heartbeat re-registration failed", zap.Error(reErr))
			}
		} else {
